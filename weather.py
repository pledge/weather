...
async def get_uk_weather_alerts(latitude: float, longitude: float) -> str:
    ...
    if current_wind > 50:
        alerts.append(
            "⚠️ HIGH WIND WARNING: Current wind speed is {:.1f} km/h. Strong winds may cause disruption.".format(
                current_wind
            )
        )
    elif current_wind > 30:
        alerts.append(
            "⚠️ WIND ADVISORY: Current wind speed is {:.1f} km/h. Be aware of gusty conditions.".format(
                current_wind
            )
        )
    ...
    if current_temp > 30:
        alerts.append(
            "🌡️ HEAT ADVISORY: Current temperature is {:.1f}°C. Stay hydrated and avoid prolonged sun exposure.".format(
                current_temp
            )
        )
    elif current_temp < 0:
        alerts.append(
            "🥶 COLD WEATHER WARNING: Current temperature is {:.1f}°C. Risk of ice and freezing conditions.".format(
                current_temp
            )
        )
    ...
    if daily:
        ...
        for i in range(min(3, len(dates))):
            ...
            if wind_max > 60:
                alerts.append(
                    "🌪️ SEVERE WIND WARNING for {}: Expected wind speeds up to {:.1f} km/h. Significant disruption possible.".format(
                        date, wind_max
                    )
                )
            elif wind_max > 40:
                alerts.append(
                    "💨 WIND WARNING for {}: Expected wind speeds up to {:.1f} km/h.".format(
                        date, wind_max
                    )
                )
            ...
            if temp_max > 32:
                alerts.append(
                    "🔥 HEAT WARNING for {}: Expected maximum temperature {:.1f}°C. Very hot conditions.".format(
                        date, temp_max
                    )
                )
            elif temp_min < -5:
                alerts.append(
                    "❄️ SEVERE COLD WARNING for {}: Expected minimum temperature {:.1f}°C. Risk of severe frost.".format(
                        date, temp_min
                    )
                )
            ...
            if precip > 25:
                alerts.append(
                    "🌧️ HEAVY RAIN WARNING for {}: Expected precipitation {:.1f} mm. Possible flooding.".format(
                        date, precip
                    )
                )
            elif precip > 15:
                alerts.append(
                    "🌦️ RAIN ADVISORY for {}: Expected precipitation {:.1f} mm.".format(
                        date, precip
                    )
                )
